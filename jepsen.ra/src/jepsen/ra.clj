...
(defrecord RaClient [conn]
  ...
  (setup! [_ test node]
    (when (= node (first (:nodes test)))
      (c/cd "/opt/ra"
        (c/exec :ra_fifo_cli :start_ra_cluster :--nodes (make-nodes (:nodes test)))))
    (RaClient. nil))

  (teardown! [_ test]
    (meh
      (c/cd "/opt/ra"
        (c/exec :ra_fifo_cli :stop_ra_cluster :--nodes (make-nodes (:nodes test))))))
    ...
    (case (:f op)
      ...
      :drain  (do
                 ...
                 (->> (repeat op)                  ; Explode drain into
                      ...
                      (map (partial dequeue! test))  ; dequeue something
